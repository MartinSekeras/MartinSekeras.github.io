...
function calculateMaxProducts() {
  ...
  for (var i = 0; i < 3; i++) {
    for (var j = 0; j < 3; j++) {
      for (var k = 0; k < 3; k++) {
        var orientationLengthwise = Math.floor(palletLength / productLength) * Math.floor(palletWidth / productWidth) * Math.floor(palletHeight / productHeight);
        var orientationSideways = Math.floor(palletLength / productWidth) * Math.floor(palletWidth / productLength) * Math.floor(palletHeight / productHeight);
        var orientationHeightwise = Math.floor(palletLength / productHeight) * Math.floor(palletWidth / productWidth) * Math.floor(palletHeight / productLength);
  
        var maxProductsOrientation = Math.max(orientationLengthwise, orientationSideways, orientationHeightwise);
  
        if (maxProductsOrientation > maxProducts) {
          maxProducts = maxProductsOrientation;
  
          if (maxProducts === orientationLengthwise) {
            orientation = "Lengthwise";
          } else if (maxProducts === orientationSideways) {
        ...
        // Rotate the product dimensions for the next iteration
        var temp = productLength;
